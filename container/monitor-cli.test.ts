...
const MOCK_SCRIPTS = {
  ...
  startup: `
    ...
    pinoLog(30, 'Local:   http://localhost:4777/');
  ...
  portConflict: `
    ...
    pinoLog(60, 'Error: listen EADDRINUSE: address already in use :::4777');
    process.exit(1);
  `,
...
describe('Integration: CLI Logs Get', () => {
  it('should include health restart events in `logs get` output', async () => {
    ...
    const serverStopsListeningScript = `
      ...
      pinoLog(30, 'Server started on ' + port);

      setTimeout(() => {
        server.stop(true);
        pinoLog(30, 'Server stopped listening on ' + port);
      }, 200);
