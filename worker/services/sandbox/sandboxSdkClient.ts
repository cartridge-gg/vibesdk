...
const SHARED_SANDBOX_PREVIEW_PORTS = Array.from(
	{ length: 10 },
	(_, index) => DEFAULT_PREVIEW_PORT + index,
);
...
function getAutoAllocatedSandbox(sessionId: string): string {
	...
	const max_instances = env.MAX_SANDBOX_INSTANCES
		? Number(env.MAX_SANDBOX_INSTANCES)
		: 10;
	const containerIndex = hash % max_instances;
	const containerId = `container-pool-${containerIndex}`;

	console.log(`Session mapped to container`, {
		sessionId,
		containerId,
		hash,
		containerIndex,
...
export class SandboxSdkClient extends BaseSandboxService {
	...
	constructor(sandboxId: string, agentId: string) {
		...
		this.logger.info('SandboxSdkClient initialized', {
			sandboxId: this.sandboxId,
		});
	}
	...
	private async getOrCreateSession(
		...
		try {
			...
			this.logger.info('Session already exists, retrieving it', {
				sessionId,
				cwd,
			});
			const existingSession =
				await this.getSandbox().getSession(sessionId);
			...
			if (actualCwd !== cwd) {
				this.logger.warn(
					'Existing session has wrong cwd, attempting to change directory',
					{
						sessionId,
						expectedCwd: cwd,
						actualCwd,
				...
				if (verifyResult.stdout.trim() !== cwd) {
					...
					this.logger.error(
						`Failed to set working directory to ${cwd}, currently at ${verifyResult.stdout.trim()}`,
					);
				}
				this.logger.info(
					'Successfully changed directory for existing session',
					{ sessionId, cwd },
				);
			}
	 ...
	 */
	private async getInstanceSession(
		instanceId: string,
		cwd?: string,
	): Promise<ExecutionSession> {
	...
	private async writeFilesViaScript(
		...
		this.logger.info('Writing files via shell script', {
		...
		try {
			...
			// Execute with bash
			const { stdout, stderr } = await session.exec(
				`bash ${scriptPath}`,
				{ timeout: 60000 },
			);
	...
	async writeFilesBulk(
		...
		try {
			...
			const writeResults = await this.writeFilesViaScript(
				filesToWrite,
				session,
			);
	...
	private async getInstanceMetadata(
		...
		try {
			const session = await this.getDefaultSession();
			const metadataFile = await session.readFile(
				this.getInstanceMetadataFile(instanceId),
			);
			...
			this.logger.error(
				`Failed to read instance metadata: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
			throw new Error(
				`Failed to read instance metadata: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
	...
	private async storeInstanceMetadata(
		instanceId: string,
		...
		if (!result.success) {
			throw new Error(
				`Failed to write instance metadata: ${result.path}`,
			);
		}
	...
	private async allocateAvailablePort(
		excludedPorts: number[] = [3000],
	): Promise<number> {
		const portCandidates =
			env.ALLOCATION_STRATEGY === AllocationStrategy.MANY_TO_ONE
				? SHARED_SANDBOX_PREVIEW_PORTS
				: [DEFAULT_PREVIEW_PORT];
		const availableCandidates = portCandidates.filter(
			(port) => !excludedPorts.includes(port),
		);

		if (availableCandidates.length === 0) {
			throw new Error(
				`No preview ports available from configured candidates: ${portCandidates.join(', ')}`,
			);
		}
		...
		if (availableCandidates.length === 1) {
			...
			this.logger.info(
				`Allocated dedicated preview port: ${allocatedPort}`,
			);
		...
		throw new Error(
			`No available ports found in configured range: ${availableCandidates.join(', ')}`,
		);
	}
	...
	private async buildFileTree(
		...
		try {
			...
			const { dirExclusions, fileExclusions } =
				FileTreeBuilder.generateFindExclusions();
			...
			const filesResult = await this.executeCommand(
				instanceId,
				buildTreeCmd,
			);
			if (filesResult.exitCode === 0) {
				return FileTreeBuilder.buildFromFindOutput(
					filesResult.stdout.trim(),
				);
			}
		} catch (error) {
	...
	async listAllInstances(): Promise<ListInstancesResponse> {
		try {
			...
			const bulkResult = await this.safeSandboxExec(
				`find . -maxdepth 1 -name "*-metadata.json" -type f -exec sh -c 'echo "===FILE:$1==="; cat "$1"' _ {} \\;`,
			);
			...
			const metadataSections: string[] = bulkResult.stdout
				.split('===FILE:')
			...
			for (const section of metadataSections) {
				try {
					...
					this.logger.warn(
						`Failed to process metadata section`,
						error,
					);
				}
			}
	...
	private async waitForServerReady(
		...
		for (let attempt = 1; attempt <= maxAttempts; attempt++) {
			try {
				...
				if (probeResult.exitCode === 0) {
					...
					this.logger.info(
						'Development server ready via HTTP probe',
						{
							instanceId,
							port,
					...
					return { ready: true, lastLogSnippet };
				}
					...
					if (logsResult.success) {
						...
						for (const pattern of readinessPatterns) {
							if (pattern.test(logs)) {
								const elapsedTime = Date.now() - startTime;
								this.logger.info(
									'Development server ready via logs',
									{
										instanceId,
										port,
										elapsedTimeMs: elapsedTime,
				...
				this.logger.warn(
					`Error checking server readiness for ${instanceId} (attempt ${attempt}):`,
					error,
				);
		...
		return { ready: false, lastLogSnippet };
	}
	...
	private async startDevServer(
		...
		try {
			...
			try {
				const readiness = await this.waitForServerReady(
					instanceId,
					process.id,
					port,
				);
				if (readiness.ready) {
					this.logger.info('Development server is ready', {
						instanceId,
					});
				} else {
				...
				this.logger.warn(
					`Error during readiness check for ${instanceId}:`,
					readinessError,
				);
	 ...
	 */
	private async provisionTemplateResources(
		instanceId: string,
		projectName: string,
	): Promise<ResourceProvisioningResult> {
		try {
			const session = await this.getInstanceSession(instanceId);

			// Read wrangler.jsonc file using absolute path
			const wranglerFile = await session.readFile(
				`/workspace/${instanceId}/wrangler.jsonc`,
			);
			if (!wranglerFile.success) {
				this.logger.info(
					`No wrangler.jsonc found for ${instanceId}, skipping resource provisioning`,
				);
				return {
					success: true,
					provisioned: [],
			...
			const parseResult = templateParser.parseWranglerConfig(
				wranglerFile.content,
			);

			if (!parseResult.hasPlaceholders) {
				this.logger.info(
					'No placeholders found in wrangler configuration',
					{ instanceId },
				);
				return {
					success: true,
			...
			this.logger.info('Placeholders found for provisioning', {
				instanceId,
				count: parseResult.placeholders.length,
			});
			...
			try {
				...
				this.logger.warn(
					`Cannot initialize resource provisioner: ${error instanceof Error ? error.message : 'Unknown error'}`,
				);
				return {
					success: true,
					provisioned: [],
			...
			for (const placeholderInfo of parseResult.placeholders) {
				this.logger.info(
					`Provisioning ${placeholderInfo.resourceType} resource for placeholder ${placeholderInfo.placeholder}`,
				);

				const provisionResult =
					await resourceProvisioner.provisionResource(
				...
				if (provisionResult.success && provisionResult.resourceId) {
					...
					replacements[placeholderInfo.placeholder] =
						provisionResult.resourceId;
				} else {
					failed.push({
						placeholder: placeholderInfo.placeholder,
						resourceType: placeholderInfo.resourceType,
					...
					this.logger.warn(
						`Failed to provision ${placeholderInfo.resourceType} for ${placeholderInfo.placeholder}: ${provisionResult.error}`,
					);
				}
			}
			...
			if (Object.keys(replacements).length > 0) {
				const updatedContent = templateParser.replacePlaceholders(
					wranglerFile.content,
					replacements,
				);
				const writeResult = await session.writeFile(
					`/workspace/${instanceId}/wrangler.jsonc`,
					updatedContent,
				);
				...
				if (writeResult.success) {
					...
					this.logger.info(
						`Updated wrangler.jsonc with ${Object.keys(replacements).length} resource IDs for ${instanceId}`,
					);
					this.logger.info(
						templateParser.createReplacementSummary(replacements),
					);
				} else {
					this.logger.error(
						`Failed to update wrangler.jsonc for ${instanceId}`,
					);
				}
			}
			...
			if (failed.length > 0) {
				this.logger.warn(
					`Resource provisioning completed with ${failed.length} failures for ${instanceId}`,
				);
			} else {
				this.logger.info(
					`Resource provisioning completed successfully for ${instanceId}`,
				);
			}
			...
			this.logger.error(
				`Exception during resource provisioning for ${instanceId}:`,
				error,
			);
			return {
				success: false,
	...
	private async startCloudflaredTunnel(
		...
		try {
			...
			const logStream = await this.getSandbox().streamProcessLogs(
				process.id,
			);
			...
			return new Promise<string>((resolve, reject) => {
				const timeout = setTimeout(() => {
					...
					this.logger.warn(
						'Timeout waiting for cloudflared tunnel URL',
					);
					resolve('');
				...
				const processLogs = async () => {
					try {
						for await (const event of parseSSEStream<LogEvent>(
							...
							if (event.data) {
								...
								if (urlMatch) {
									...
									this.logger.info(
										`Found cloudflared tunnel URL: ${previewURL}`,
									);
									resolve(previewURL);
									return;
								}
						...
						this.logger.error(
							'Cloudflare tunnel process failed',
							error,
						);
						clearTimeout(timeout);
						reject(error);
	...
	private async updateProjectConfiguration(
		...
		try {
			...
			const packageJsonResult = await session.exec(
			...
			this.logger.info(
				`Updating wrangler.jsonc with project name: ${projectName}`,
			);
			const wranglerResult = await session.exec(
				`sed -i '0,/"name":/s/"name"[ ]*:[ ]*"[^"]*"/"name": "${projectName}"/' wrangler.jsonc`,
			);

			if (wranglerResult.exitCode !== 0) {
				this.logger.warn(
					'Failed to update wrangler.jsonc',
	...
	private async setLocalEnvVars(
		...
		try {
			...
			const envVarsContent = Object.entries(localEnvVars)
				...
				.join('\n');
			const result = await session.writeFile(
				`/workspace/${instanceId}/.dev.vars`,
				envVarsContent,
			);
			if (!result.success) {
				throw new Error('Failed to write .dev.vars file');
			}
			this.logger.info('Environment variables written to .dev.vars', {
				instanceId,
				varCount: Object.keys(localEnvVars).length,
			});
		} catch (error) {
			this.logger.error(
				`Error setting local environment variables: ${error}`,
			);
			throw error;
		}
	}
	...
	> {
		try {
			...
			const resourceProvisioningResult =
				await this.provisionTemplateResources(instanceId, projectName);
			if (
				!resourceProvisioningResult.success &&
				resourceProvisioningResult.failed.length > 0
			) {
				this.logger.warn(
					`Some resources failed to provision for ${instanceId}, but continuing setup process`,
				);
			}
			...
			try {
				const session = await this.getInstanceSession(instanceId);
				const wranglerConfigFile = await session.readFile(
					`/workspace/${instanceId}/wrangler.jsonc`,
				);
				if (wranglerConfigFile.success) {
					await env.VibecoderStore.put(
						this.getWranglerKVKey(instanceId),
						wranglerConfigFile.content,
					);
					this.logger.info('Wrangler configuration stored in KV', {
						instanceId,
					});
				} else {
					this.logger.warn(
						'Could not read wrangler.jsonc for KV storage',
						{ instanceId },
					);
				}
			} catch (error) {
				this.logger.warn('Failed to store wrangler config in KV', {
					instanceId,
					error:
			...
			if (isDev(env) || env.USE_TUNNEL_FOR_PREVIEW) {
				this.logger.info(
					'Starting cloudflared tunnel for local development',
					{ instanceId },
				);
				tunnelUrlPromise = this.startCloudflaredTunnel(
					instanceId,
					allocatedPort,
				);
			}
			...
			if (installResult.exitCode === 0) {
				...
				try {
					...
					const processId = await this.startDevServer(
						instanceId,
						initCommand,
						allocatedPort,
					);
					...
					const previewResult = await sandbox.exposePort(
						allocatedPort,
					...
					if (env.USE_TUNNEL_FOR_PREVIEW) {
						this.logger.info(
							'Using tunnel url instead for preview as configured',
							{ instanceId, tunnelURL },
						);
						previewURL = tunnelURL;
					}

					this.logger.info('Preview URL exposed', {
						instanceId,
						previewURL,
					});
	...
	async createInstance(
		...
		const { files, projectName, webhookUrl, envVars, initCommand } =
			options;
		try {
			// Environment variables will be set via session creation on first use
			if (envVars && Object.keys(envVars).length > 0) {
				this.logger.info(
					'Environment variables will be configured via session',
			...
			if (env.ALLOCATION_STRATEGY === 'one_to_one') {
				...
				if (
					instancesResp.success &&
					instancesResp.instances.length > 0
				) {
					this.logger.error(
						'There are already instances running in sandbox, creating a new instance may cause issues',
					...
					if (instanceStatus.success && instanceStatus.isHealthy) {
						this.logger.error(
							'Instance already exists and is active, creating a new instance may cause issues',
						...
						this.logger.error(
							'Instance already exists but is not active, Removing old instance',
							{ instance: firstInstance },
						);
			...
			const dontTouchFile = files.find(
				(f) => f.filePath === '.donttouch_files.json',
			);
			const dontTouchFiles = dontTouchFile
				? JSON.parse(dontTouchFile.fileContents)
				: [];

			const redactedFile = files.find(
				(f) => f.filePath === '.redacted_files.json',
			);
			const redactedFiles = redactedFile
				? JSON.parse(redactedFile.fileContents)
				: [];
			...
			const results = await this.setupInstance(
				instanceId,
				projectName,
				initCommand,
				envVars,
			...
			this.logger.error(
				`Failed to create instance for project ${projectName}`,
				{
					error,
					projectName,
			...
			try {
				...
				if (metadata.processId) {
					for (let i = 0; i < 3; i++) {
						try {
							const processes =
								await this.getSandbox().listProcesses();
							const process = processes.find(
								(p: { id: string; status: string }) =>
									p.id === metadata.processId,
							);
							isHealthy = !!(
							...
							this.logger.error(
								`Failed to check process ${metadata.processId}, retrying...${i + 1}/3`,
								{ error },
							);
		...
		try {
			...
			if (metadata.processId) {
				try {
					...
					this.logger.warn(
						`Failed to kill process ${metadata.processId}`,
						error,
					);
				}
			}
			...
			if (metadata.allocatedPort) {
				try {
					...
					this.logger.info(
						`Unexposed port ${metadata.allocatedPort} for instance ${instanceId}`,
					);
				} catch (error) {
					this.logger.warn(
						`Failed to unexpose port ${metadata.allocatedPort}`,
						error,
					);
				}
			}
	...
	async writeFiles(
		...
		try {
			...
			const filteredFiles = files.filter(
				(file) => !donttouchFiles.has(file.filePath),
			);
			const rawResults = await this.writeFilesBulk(
				instanceId,
				filteredFiles,
			);
			...
			const wereDontTouchFiles = files.filter((file) =>
				donttouchFiles.has(file.filePath),
			);
			...
			if (wereDontTouchFiles.length > 0) {
				this.logger.warn(
					'Files were not written (protected by donttouch_files)',
			...
			if (
				...
				this.logger.info(
					'Touching .reload-trigger to trigger page reload',
				);
	...
	async getFiles(
		...
		try {
			...
			if (!filePaths) {
				...
				const importantFiles = await session.exec(
					`jq -r '.[]' .important_files.json | while read -r path; do if [ -d "$path" ]; then find "$path" -type f; elif [ -f "$path" ]; then echo "$path"; fi; done`,
				);
				this.logger.info(
					`Read important files: stdout: ${importantFiles.stdout}, stderr: ${importantFiles.stderr}`,
				);
				filePaths = importantFiles.stdout
					.split('\n')
				...
				this.logger.info(
					`Successfully read important files: ${filePaths}`,
				);
				applyFilter = true;
			}
			...
			if (applyFilter) {
				if (redactedFiles) {
					...
					try {
						const metadata =
							await this.getInstanceMetadata(
								templateOrInstanceId,
							);
						redactedPaths = new Set(metadata.redacted_files);
					} catch (error) {
						this.logger.warn('Failed to get redacted files', {
							templateOrInstanceId,
						});
					}
				}
			...
			const readPromises = filePaths.map(async (filePath: string) => {
				try {
					const result = await session.readFile(
						`/workspace/${templateOrInstanceId}/${filePath}`,
					);
					return {
						result,
						filePath,
				...
				if (readResult.status === 'fulfilled') {
					...
					if (result && result.success) {
						files.push({
							filePath: filePath,
							fileContents:
								applyFilter && redactedPaths.has(filePath)
									? '[REDACTED]'
									: result.content,
						});
		...
		try {
			...
			for (const command of commands) {
				try {
					...
					this.logger.error('Command execution failed with error', {
						command,
						error,
					});
					results.push({
		...
		try {
			...
			if (result.exitCode === 0) {
				...
				try {
					...
					this.logger.info(
						`getInstanceErrors - ${response.errors.length ? 'errors found' : ''}: ${result.stdout}`,
					);
				} catch (parseError) {
					this.logger.warn('Failed to parse CLI output as JSON', {
						stdout: result.stdout,
					});
			...
			this.logger.error(
				`Failed to get errors for instance ${instanceId}: STDERR: ${result.stderr}, STDOUT: ${result.stdout}`,
			);
				...
				if (result.exitCode === 0) {
					...
					if (response.success) {
						return {
							...
							message:
								response.message ||
								`Cleared ${response.clearedCount || 0} errors`,
						};
					}
				}
			} catch (enhancedError) {
				this.logger.warn(
					'Error clearing unavailable, falling back to legacy',
					enhancedError,
				);
			}
		...
		try {
			...
			if (lintResult.status === 'fulfilled') {
					...
					for (const fileResult of lintData) {
						for (const message of fileResult.messages || []) {
							lintIssues.push({
								...
								severity: this.mapESLintSeverity(
									message.severity,
								),
				...
				results.lint.summary = {
					errorCount: lintIssues.filter(
						(issue) => issue.severity === 'error',
					).length,
					warningCount: lintIssues.filter(
						(issue) => issue.severity === 'warning',
					).length,
					infoCount: lintIssues.filter(
			...
			if (tscResult.status === 'fulfilled') {
				try {
					...
					const output =
						tscResult.value.stderr || tscResult.value.stdout;

					if (!output || output.trim() === '') {
						this.logger.info('No TypeScript output to parse');
					} else {
						this.logger.info(
							`Parsing TypeScript output: ${output.substring(0, 200)}...`,
						);
						...
						for (const line of lines) {
							...
							if (match) {
								...
								this.logger.info(
									`Found TypeScript error: ${currentError.filePath}:${currentError.line} - ${currentError.ruleId}`,
								);
							} else if (
								currentError &&
								line.trim() &&
						...
						this.logger.info(
							`Parsed ${typecheckIssues.length} TypeScript errors`,
						);
					}
				} catch (error) {
					this.logger.warn(
						'Failed to parse TypeScript output',
						error,
					);
				}
				...
				results.typecheck.summary = {
					errorCount: typecheckIssues.filter(
						(issue) => issue.severity === 'error',
					).length,
					warningCount: typecheckIssues.filter(
						(issue) => issue.severity === 'warning',
					).length,
					infoCount: typecheckIssues.filter(
						(issue) => issue.severity === 'info',
					).length,
				};
			...
			this.logger.info(
				`Analysis completed: ${lintIssues.length} lint issues, ${typecheckIssues.length} typecheck issues`,
			);
	...
	async deployToCloudflareWorkers(
		...
		try {
			...
			if (!wranglerConfigContent) {
				...
				throw new Error(
					`Wrangler config not found in KV for ${instanceId}`,
				);
			} else {
			...
			this.logger.info('Worker configuration', {
				scriptName: config.name,
			});
			this.logger.info('Worker compatibility', {
			...
			const session = await this.getInstanceSession(instanceId);
			const workerFile = await session.readFile(
				`/workspace/${instanceId}/dist/index.js`,
			);
			if (!workerFile.success) {
				throw new Error(
					`Worker script not found at /${instanceId}/dist/index.js. Please build the project first.`,
				);
			}
			...
			try {
				...
				const workerAssetsResult = await this.safeSandboxExec(
					`test -d ${workerAssetsPath} && echo "exists" || echo "missing"`,
				);
				const hasWorkerAssets =
					workerAssetsResult.exitCode === 0 &&
					workerAssetsResult.stdout.trim() === 'exists';

				if (hasWorkerAssets) {
					this.logger.info('Processing additional worker modules', {
						workerAssetsPath,
					});

					// Find all JS files in the worker assets directory
					const findResult = await this.safeSandboxExec(
						`find ${workerAssetsPath} -type f -name "*.js"`,
					);
					if (findResult.exitCode === 0) {
						const modulePaths = findResult.stdout
							.trim()
							.split('\n')
						...
						if (modulePaths.length > 0) {
							...
							for (const fullPath of modulePaths) {
								...
								try {
									const buffer =
										await this.readFileAsBase64Buffer(
											fullPath,
										);
									const moduleContent =
										buffer.toString('utf8');
									additionalModules.set(
										relativePath,
										moduleContent,
									);
									...
									this.logger.info('Worker module loaded', {
										path: relativePath,
										sizeKB: (
											moduleContent.length / 1024
										).toFixed(2),
									});
								} catch (error) {
									this.logger.warn(
										`Failed to read worker module ${fullPath}:`,
										error,
									);
								}
							}
							...
							if (additionalModules.size > 0) {
								this.logger.info(
									'Found additional worker modules',
				...
				this.logger.error(
					'Failed to process additional worker modules:',
					error,
				);
			}
			...
			const assetDirResult = await this.safeSandboxExec(
				`test -d ${assetsPath} && echo "exists" || echo "missing"`,
			);
			const hasAssets =
				assetDirResult.exitCode === 0 &&
				assetDirResult.stdout.trim() === 'exists';

			if (hasAssets) {
				this.logger.info('Processing static assets', { assetsPath });
				const assetProcessResult = await this.processAssetsInSandbox(
					instanceId,
					assetsPath,
				);
				...
				this.logger.info(
					'No static assets found, deploying worker only',
				);
			}
	 ...
	 */
	private async processAssetsInSandbox(
		_instanceId: string,
		assetsPath: string,
	): Promise<{
		...
		for (const fullPath of filePaths) {
			...
			try {
				...
				this.logger.info('Asset file processed', {
					path: relativePath,
					sizeBytes: buffer.length,
				});
			} catch (error) {
				this.logger.warn(
					`Failed to read asset file ${fullPath}:`,
					error,
				);
			}
		}
	...
	private async readFileAsBase64Buffer(filePath: string): Promise<Buffer> {
		...
		const base64Result = await this.safeSandboxExec(
			`base64 -w 0 "${filePath}"`,
		);
	...
	private getProtocolForHost(): string {
		...
		if (
			...
			previewDomain.includes('127.0.0.1')
		) {
			return 'http';
		}
		return 'https';
