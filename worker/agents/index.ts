...
import { isGameTemplate } from './utils/gamePlatform';
...
export async function getAgentStub(
	...
	return getAgentByName<Env, CodeGeneratorAgent>(
		env.CodeGenObject,
		agentId,
		options,
	);
...
export async function getAgentStubLightweight(
	env: Env,
	agentId: string,
): Promise<DurableObjectStub<CodeGeneratorAgent>> {
...
export async function cloneAgent(
	...
	newAgentId: string;
	newAgent: DurableObjectStub<CodeGeneratorAgent>;
}> {
	const agentInstance = await getAgentStub(env, agentId);
...
async function handleUserSelectedTemplate(
	...
	if (!templatesResponse?.success) {
		throw new Error(
			`Failed to fetch templates from sandbox service, ${templatesResponse.error}`,
		);
	}

	const matchedTemplate = templatesResponse.templates.find(
		(t) => t.name === templateName,
	);
	if (!matchedTemplate) {
		throw new Error(
			`Specified template '${templateName}' not found in available templates`,
		);
	}

	if (!isGameTemplate(matchedTemplate)) {
		throw new Error(
			`Template '${templateName}' is not available in game-only mode. Select a game-oriented template instead.`,
		);
	}

	const templateDetailsResponse = await BaseSandboxService.getTemplateDetails(
		matchedTemplate.name,
	);
	if (
		!templateDetailsResponse.success ||
		!templateDetailsResponse.templateDetails
	) {
		throw new Error(
			`Failed to fetch template details for '${templateName}'`,
		);
	}
...
async function handleAITemplateSelection(
	args: Omit<TemplateQueryArgs, 'selectedTemplate'>,
): Promise<TemplateQueryResult> {
	...
	const matchedTemplate = templatesResponse.templates.find(
		(t) => t.name === aiSelection.selectedTemplateName,
	);
	if (!matchedTemplate) {
...
export async function getTemplateForQuery(
	...
	return handleAITemplateSelection({
		env,
		inferenceContext,
		query,
		projectType,
