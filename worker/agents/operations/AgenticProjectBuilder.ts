...
const getUserPrompt = (
	...
	const normalizedQuery = appendGamePlatformQuery(query);
...
> {
	protected getCallbacks(
		...
		const { streamCb, toolRenderer, onToolComplete, onAssistantMessage } =
			inputs;
		return {
			streamCb,
			toolRenderer,
	...
	protected buildSession(
		...
		const hasTSX =
			filesIndex?.some((f) => /\.(t|j)sx$/i.test(f.filePath)) || false;
		const hasMD =
		...
		const dynamicHints = [
			'- Game-only platform: build a game and nothing else. If the user prompt is not explicitly a game, reinterpret its theme into a game concept while preserving the original intent.',
			!hasPlan
			...
			needsSandbox && !hasTemplate
				? '- Game project without template: Use init_suitable_template() to let AI select and import the best game-oriented template before first deploy.'
				: '',
			...
			hasMD && !hasTSX
				...
				: '',
			!hasFiles && hasPlan
				? '- Plan ready, no files yet: Scaffold the initial game structure with generate_files.'
				: '',
		]
			.filter(Boolean)
			.join('\n');
	...
	protected async buildMessages(
		...
		if (
			...
			const prepared = await prepareMessagesForInference(
				env,
				inputs.conversationHistory,
			);
